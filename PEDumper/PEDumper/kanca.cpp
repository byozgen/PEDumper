...
bool kanca::add_redirect(unsigned __int64 target_address)
{
...
unsigned __int64 kanca::get_address(char * library, char * procedure_name)
{
	return 0;
}

bool kanca::get_terminate_orig_code(unsigned char * out_buffer, int length)
{
	return false;
}

bool kanca::hook_terminate(export_list* exports)
{
			...
			else
			{
				...
				if (changed)
				{
					...
					if (num_read == sizeof(_original_hook_bytes) && success)
					{
						...
						if (!_is64)
						{
							// Windows 10/11, wow64 version of ZwTerminateProcess
						...
						else
						{
							...
							*((unsigned __int64*)(code64 + 0x30)) = 32; // _hook_original_code_length
							memcpy(code64 + 0x38, _original_hook_bytes, 32); // _hook_original_code
...
bool kanca::is_terminate_waiting()
{
...
kanca::kanca(HANDLE ph, DWORD pid, bool is64, PD_OPTIONS* options)
{
	_ph = ph;
	_pid = pid;
	_is64 = is64;
