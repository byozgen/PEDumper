...
//MZ
bool databasePE::_is_mz(FILE* stream)
{
	char twochars[2];
...
databasePE::databasePE(char* clean_database_name)
{
...
bool databasePE::clear_database()
{
	EnterCriticalSection(&_lock);
	_clean_hashes.clear();
	LeaveCriticalSection(&_lock);
...
bool databasePE::add_hashes(unordered_set<unsigned __int64> hashes)
{
	EnterCriticalSection(&_lock);
...
bool databasePE::add_folder(char* dir_name, WCHAR* filter, bool recursively)
{
...
bool databasePE::remove_folder(char* dir_name, WCHAR* filter, bool recursively)
{
...
bool databasePE::contains(unsigned __int64 hash)
{
	return _clean_hashes.count(hash) != 0;
}

bool databasePE::add_file(char* file)
{
	PD_OPTIONS options;
...
bool databasePE::remove_file(char* file)
{
	PD_OPTIONS options;
	options.ForceGenHeader = false;
	options.ImportRec = false;
